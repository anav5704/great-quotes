...
export const Like = pgTable("Like", {
	id: text("id").primaryKey().notNull(),
	userId: text("userId").notNull().references(() => User.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	quoteId: text("quoteId").notNull().references(() => Quote.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	createdAt: timestamp("createdAt", { precision: 3, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updatedAt", { precision: 3, withTimezone: true, mode: 'string' }).defaultNow().notNull(),
},
(table) => {
	return {
		userId_idx: index("Like_userId_idx").on(table.userId),
		quoteId_idx: index("Like_quoteId_idx").on(table.quoteId),
	}
});